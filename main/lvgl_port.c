 ...
 *
 * DMA 가 직접 읽는다. PSRAM 에 둬도 esp_lcd 가 읽기는 하지만, 정렬이나 플래그가
 * 어긋나면 SPI 드라이버가 내부 SRAM 에 바운스 버퍼를 잡고 매 전송마다 복사한다
 * (`spi_master.c:1191`). 내부 SRAM 은 어차피 나가고 memcpy 만 얹히므로 처음부터
 * 내부에 두는 것이 맞다 — 자세한 근거는 `mem_place.h` 의 MEM_INTENT_DMA.
